...
public class Gmail_Login_Excel2 {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		sheet.getRow(0).createCell(last).setCellValue(new Date().toString());
		...
		for (int i = 1; i <= totalrows; i++) {
			...
			if (driver.findElement(By.xpath("//input[@type='password']")).isDisplayed()) {
				...
				if (title.contains(expected)) {
					...
					sheet.getRow(i).createCell(last).setCellValue("Test Passed");
				} 
					...
					if (passwordMsg.contains(expected)) {
						...
						sheet.getRow(i).createCell(last).setCellValue("Test Failed");
					}
				}
				else {
					sheet.getRow(i).createCell(5).setCellValue(title);
